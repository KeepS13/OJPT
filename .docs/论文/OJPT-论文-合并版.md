...
随着程序设计课程实践、算法训练和编程竞赛活动的持续开展，在线判题系统在代码评测、结果反馈和训练过程记录方面具有重要作用。传统人工评阅方式难以及时处理大量代码提交，也不便于持续跟踪学习者的训练情况。针对个人算法训练和题库维护需求，本文设计并实现了OJPT在线编程训练平台，为用户提供在线练习、代码评测和训练数据查看等功能，为管理员提供题库和判题环境维护能力。

本文具体介绍了OJPT在线编程训练平台的开发设计过程，包括系统需求分析、总体架构设计、数据库设计、核心功能实现以及系统测试。系统主要实现了用户登录注册、题库浏览、在线做题、代码运行、提交判题、提交结果查询、个人中心与训练看板、后台用户管理、后台题目与测试用例维护和判题环境检测等功能。普通用户可以完成从选择题目、编辑代码、运行样例到正式提交并查看训练情况的完整流程，管理员可以完成用户状态维护、题目信息维护和判题环境检查等管理操作。

本系统采用前后端分离架构，后端使用SpringBoot、Spring Security、MyBatis-Plus、MySQL、Redis和Docker等技术实现业务接口、权限控制、数据持久化和代码隔离执行，前端使用Vue 3、TypeScript、Element Plus、Pinia和Axios构建交互页面。系统采用JWT进行身份认证，通过角色权限控制区分普通用户和管理员操作。经过功能测试、接口测试、判题流程测试、权限测试、判题环境检测和自动化测试，结果表明系统主要功能能够正常运行，满足毕业设计阶段对在线判题平台功能实现和测试验证的要求。
...
This thesis presents the development and design process of OJPT, including requirement analysis, overall architecture design, database design, core function implementation, and system testing. The system implements user login and registration, problem browsing, online solving, code running, judging submission, submission result query, user center and training dashboard, administrator user management, problem and test case maintenance, and judge environment checking. Normal users can complete the workflow from selecting a problem, editing code, and running samples to submitting code and viewing training results. Administrators can maintain user status, problem information, and the judging environment.

OJPT adopts a frontend-backend separation architecture. The backend uses SpringBoot, Spring Security, MyBatis-Plus, MySQL, Redis, and Docker to implement business APIs, permission control, data persistence, and isolated code execution. The frontend uses Vue 3, TypeScript, Element Plus, Pinia, and Axios to build interactive pages. JWT is used for identity authentication, and role-based access control is used to separate normal user operations from administrator operations. Through functional testing, API testing, judging process testing, permission testing, judge environment checking, and automated testing, the results show that the main functions of the system run properly and meet the graduation project requirements for online judge function implementation and test verification.
...
第五，完成软件工程建模和测试验证。本文根据系统实际功能绘制功能模块图、顶层数据流图、零层数据流图、核心业务一层数据流图、总体架构图、部署结构图、实体属性图、数据库E-R图、JWT鉴权流程图和核心业务流程图，并通过功能测试、接口测试、权限测试、判题流程测试和自动化测试验证系统主要功能。

## 1.4技术路线与研究方法

本文采用“需求分析—系统设计—系统实现—系统测试”的技术路线。需求分析阶段根据普通用户和管理员两类角色梳理功能需求、非功能需求和数据流关系；系统设计阶段确定前后端分离架构、部署结构、核心模块、数据库模型、鉴权机制和判题流程；系统实现阶段按照认证与账号、普通用户、做题与判题、管理端四类模块进行开发；系统测试阶段通过页面操作、接口响应、自动化测试和构建检查验证系统可用性。

后端技术路线以SpringBoot为核心，使用Spring Web暴露REST接口，使用Spring Security和JWT实现登录认证和角色访问控制，使用MyBatis-Plus进行数据访问和分页查询，使用Flyway管理数据库迁移，使用MySQL存储业务数据，使用Redis存储刷新令牌和访问令牌黑名单，使用Docker执行不同语言的用户代码。

前端技术路线以Vue 3和TypeScript为核心，使用Vue Router组织页面路由，使用Pinia维护用户登录状态，使用Axios封装接口请求，使用Element Plus构建页面组件，使用Markdown解析与DOM清洗工具展示题面内容。做题页围绕代码编辑、测试用例、运行结果和提交结果组织交互，管理端围绕列表、表单、弹窗和状态操作组织交互。

测试方法方面，本文采用手工验证和自动化测试结合的方式。手工验证主要用于截图和功能说明材料整理，覆盖登录、注册、题库筛选、做题、提交、个人中心和管理端维护等页面。自动化测试包括后端JUnit测试、前端Vitest单元测试和Playwright端到端测试，用于验证关键服务逻辑、前端组件交互、接口参数组织和用户可见流程。
