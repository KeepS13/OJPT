...
管理员后台会检查 Docker 可执行文件、Docker Engine 连接、`docker version`、`docker info` 和三类语言镜像是否可用。
...
本章已对照 `application.properties`、`vite.config.ts`、`docker-compose.yml` 和部署 Dockerfile/README 复核，覆盖前端 Vite 启动、后端 Spring Boot 启动、MySQL、Redis、Docker Engine、C/C++/Java/Python 判题镜像、Docker Compose 服务编排、Flyway 初始化数据、默认账号、上传目录与头像 URL 前缀。
